...
- 若运行环境到 Bangumi 网络不佳（如连接超时），可设置代理环境变量
  ``BANGUMI_PROXY`` 或 ``BILIBILI_PROXY``（如 http://127.0.0.1:7890），
...
DEBUG: bool = os.environ.get("BILIBILI_BANGUMI_DEBUG", "0") == "1"
...
# 代理：显式代理优先取 BANGUMI_PROXY / BILIBILI_PROXY；
# 未设置时 requests 会信任系统环境变量 HTTP_PROXY / HTTPS_PROXY
PROXY: str = (
    os.environ.get("BANGUMI_PROXY")
    or os.environ.get("BILIBILI_PROXY", "")
).strip()
