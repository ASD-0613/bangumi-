...
class MainWindow(QMainWindow):
    ...
    def on_rank_refresh(self) -> None:
        ...
        self._busy(
            f"正在获取「{category.get('label', '')}」· 按「{sort_key}」榜单…"
        )
    ...
    def _on_rank_result(self, result: Any) -> None:
        ...
        self.statusBar().showMessage(
            f"「{self.rank_combo.currentText()}」排行榜共 "
            f"{len(self._rank_items)} 条"
        )
    ...
    def _render_timeline_day(self) -> None:
        ...
        if day is None:
            self.statusBar().showMessage(
                f"{config.WEEKDAY_NAMES[self._timeline_selected - 1]}暂无更新条目"
            )
            return
    ...
    def on_timeline_item_open(self, item: QTreeWidgetItem, _col: int) -> None:
        ...
        if season_id is not None:
            ...
            QMessageBox.information(self, "提示", "请双击具体某部番剧查看详情")
